...
def run_single(item):
    ...
    print(f"[START] {ticker} ({trade_date}) - {group}")
    cmd = [
    ...
    if os.path.exists(eval_f) and os.path.exists(sig_f):
        ...
        with open(sig_f) as f:
            s = json.load(f)
        print(f"[DONE] {ticker}: Sinyal={s.get('action')} | Mode={s.get('entry_mode')} | Ret={e.get('realized_return_pct')}% | Outcome={e.get('outcome')}")
        return (ticker, True)
    else:
...
print("Completed 10 IDX Re-runs:", results)
